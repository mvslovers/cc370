...
* it and produces one LD per symbol. as370 used to take the whole
* operand as a
* single symbol name, so the >8-character external-symbol check fired
* on
* "COBDISP,COBTERM,COBWRL,COBDATE" and the module would not assemble.
* Paired with entry_list_1pl.s -- the one-ENTRY-per-line spelling of
* the same
* module, which always assembled correctly and is the form the
* IFOX-pinned
