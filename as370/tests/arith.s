*---------------------------------------------------------------
* cc370 issue #64 -- SRP, and a second full IFOX00 oracle.
*
...
* occurrences, which is what makes it the regression for #64 -- as370
* wrote SRP's length into the low nibble instead of the high one and
* never parsed the rounding digit at all, so every one of the five came
* out F0 07 where IFOX00 emits F0 70.
*
* It also carried #61: COBWS is 124 bytes at 0x500 and ends at 0x57C,
* so COBRT's origin rounds up to 0x580.  With both fixed this module is
* byte-identical to IFOX00 across every card before the END card.
*
* Needs SYS1.MACLIB's SPIE and TIME (mvslovers/libc370#155); the test
* skips without them.
*---------------------------------------------------------------
*---------------------------------------------------------------
* Generated by cobc370 from arith.cbl
...
SPIELTB  DS    0H
         DC    AL2(T0000-COBBEG),AL2(18)
         DC    AL2(T0001-COBBEG),AL2(19)
         DC    AL2(T0002-COBBEG),AL2(20)
         DC    AL2(T0003-COBBEG),AL2(21)
         DC    AL2(T0004-COBBEG),AL2(22)
         DC    AL2(T0005-COBBEG),AL2(23)
