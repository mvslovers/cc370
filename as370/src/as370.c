...
static int emit_stmt_export(const char *fn, const char *srcfn, char **lines, int nl) {
    ...
    fputs("#note\tone record per STATEMENT the assembly generated, in listing order. stmt is the\n"
          ...
          "#note\tTO COUNT PURE ALIGNMENT, reserves=0 IS NOT THE WHOLE RULE: it covers padding\n"
          "#note\tthe assembler inserted (DS 0F, CNOP) AND bytes an ORG skipped, which nobody\n"
          "#note\towns. Require reserves=0 AND len>0 AND the operation is not ORG. Measured over\n"
          "#note\t30 modules: the full rule gives 412 bytes, dropping the ORG clause gives 1,430,\n"
          "#note\tand summing len over every reserves=0 record gives -21,907.\n"
