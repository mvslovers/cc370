...
expand_builtin_strchr (tree arglist, rtx target, enum machine_mode mode)
{
  if (!validate_arglist (arglist, POINTER_TYPE, INTEGER_TYPE, VOID_TYPE))
    ...
    {
      tree s1 = TREE_VALUE (arglist), s2 = TREE_VALUE (TREE_CHAIN (arglist));
      const char *p1 ATTRIBUTE_UNUSED;
	...
	return 0;
      ...
      /* On an EBCDIC target this constant fold is invalid: c_getstr returns
	 the source (host, ASCII) bytes of S1, but S2 is the target (EBCDIC)
	 character constant -- the two charsets differ, so searching ASCII
	 bytes for an EBCDIC code point gives the wrong answer.  Fall through
	 to a runtime strchr() call, which compares EBCDIC against EBCDIC.  */
      if (p1 != NULL)
	{
	  char c;
	  const char *r;
...
expand_builtin_strrchr (tree arglist, rtx target, enum machine_mode mode)
{
  if (!validate_arglist (arglist, POINTER_TYPE, INTEGER_TYPE, VOID_TYPE))
	...
	return 0;
      ...
      p1 = c_getstr (s1);
#ifndef TARGET_EBCDIC
      /* See expand_builtin_strchr: invalid on EBCDIC targets (host-ASCII
	 string bytes vs target-EBCDIC character constant).  Use a runtime
	 call.  The strrchr(s1,'\0') -> strchr transform below is fine ('\0'
	 is 0 in every charset).  */
      if (p1 != NULL)
	{
	  char c;
	  const char *r;
...
expand_builtin_memcpy (tree arglist, rtx target, enum machine_mode mode)
{
  if (!validate_arglist (arglist,
  ...
  else
    {
      ...
      const char *src_str ATTRIBUTE_UNUSED;
...
#ifndef I370_NO_INLINE_STRING_CONST
      if (src_str
...
#endif /* I370_NO_INLINE_STRING_CONST: emit EBCDIC literal via emit_block_move */
...
#endif /* I370_NO_INLINE_STRING_CONST: copy reads EBCDIC literal below */
...
expand_builtin_strncpy (tree arglist, rtx target, enum machine_mode mode)
{
  if (!validate_arglist (arglist,
	...
	{
#ifdef I370_NO_INLINE_STRING_CONST
	  /* Do not materialise the string constant via store_by_pieces here:
	     builtin_strncpy_read_str copies the host (ASCII) bytes, whereas the
	     constant pool is assembled as EBCDIC.  Punt to the library strncpy,
	     which copies the in-memory (EBCDIC) literal.  */
	  return 0;
#else
	  tree dest = TREE_VALUE (arglist);
	  unsigned int dest_align
	  ...
	  dest_mem = convert_memory_address (ptr_mode, dest_mem);
	  return dest_mem;
#endif /* I370_NO_INLINE_STRING_CONST */
	}
