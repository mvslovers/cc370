...
;;   %W -- Print a 64-bit constant as an 8-byte hex literal.
;;
...
;
; The GNU gas assembler doesn't like 'BH  *+12' because it thinks that
  ...
  "*
{
  ...
  if (REG_P (operands[0]))
  	  ...
  	  && (unsigned long) INTVAL (operands[1]) < 4096)
	{
      ...
      if (GET_CODE (operands[1]) == CONST_INT
	  && INTVAL (operands[1]) == (HOST_WIDE_INT) (int) INTVAL (operands[1]))
	{
      ...
      if (GET_CODE (operands[1]) == CONST_INT)
	{
	  /* Not the sign-extension of its low word: L + SRDA would drop
	     the high word, so load both words from an 8-byte literal.  */
	  mvs_check_page (0, 4, 8);
	  return \"LM	%0,%N0,%W1\";
	}
      if (GET_CODE (operands[1]) == CONST_DOUBLE)
	{
      ...
      mvs_check_page (0, 4, 0);
      /*return \"LM	%0,%N0,%1\";*/
      return i370_output_load_pair (operands);
    }
...
;
; The simple solution is to just comment out the whole insn definition.
...
;; However, MS and MSR are insns first introduced in z/Architecture
;; and are not available on System/370. So, for now, we don't do this.
;;
;;(define_insn "mulsi3"
;;  [(set (match_operand:SI 0 "register_operand" "=d")
  ...
  "*
{
  check_label_emit ();
  mvs_check_page (0, 4, 2);
  /* In z/Architecture, we could use MHI instead. */
  return \"MH	%0,%H2\";
}"
   [(set_attr "length" "4")]
)
...
{
  check_label_emit ();
  /* SLDL, not SLDA: a C left shift is logical on the bit pattern, and
   * SLDA keeps the sign bit, losing bit 63 (issue #468).  SLDL leaves
   * the condition code alone, so no CC_STATUS_SET here.  */
  mvs_check_page (0, 4, 0);
  if (REG_P (operands[2]))
    return \"SLDL	%0,0(%2)\";
  return \"SLDL	%0,%c2\";
