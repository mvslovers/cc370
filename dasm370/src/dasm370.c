...
static int  reach_apply;                      /* the disassembly obeys the traversal */
...
/* The coverage card, and it is the acceptance rather than a decoration: with the
 * traversal reaching a median of 97.7 % and an aggregate of 82.2 % over the 30
 * control CSECTs, a module where it found everything and one where it found a
 * root and stopped are otherwise the same disassembly.  A comment card, because
 * the gate skips column 1 -- which it does only because the first version of
 * this line was classified as an instruction at whatever six-hex number it
 * mentioned. */
static void reach_card(void)
{
    char t[200];
    snprintf(t, sizeof t,
             "REACHABILITY roots=%d sd=%d ld=%d end=%d acon=%d/%dtab  "
             "reached=%ld dark=%ld runs=%ld  bases balr=%d lr=%d rld=%d  set=%s%s%s%s",
             nr_sd + nr_ld + nr_end, nr_sd, nr_ld, nr_end, nr_acon, nr_acontab,
             reach_reached, sect_len - reach_reached, reach_runs,
             nr_balrbase, nr_lrbase, nr_rldbase,
             (reach_mode & RCH_R15) ? "r15 " : "",
             (reach_mode & RCH_BALR) ? "balr " : "",
             (reach_mode & RCH_LR) ? "lr " : "",
             (reach_mode & RCH_ACON) ? "acon" : "");
    emit_comment(t);
}
...
static void walk_section(void)
{
    ...
    while (a < sect_len) {
        ...
        if (!hin_data(a)) {
            ...
            /* #383: nothing reached this byte, so it is not entered here and it
             * is not an instruction.  CONSERVATIVE BY CONSTRUCTION -- unreached
             * CODE also becomes DC, which is byte-safe and therefore invisible
             * to the round trip, and is why the coverage card below is the
             * acceptance rather than an accessory to it. */
            if (reach_apply && !rch[a]) o = NULL;
            len = o ? ins_len_of(o->fmt) : 0;
...
int main(int argc, char **argv)
{
    ...
    for (ai = 1; ai < argc; ai++) {
        if (!strcmp(argv[ai], "--help")) { usage(stdout); return 0; }
            ...
            else { reach_apply = 1; v = argv[ai] + 7; }
    ...
    /* #383: the traversal runs BEFORE the walk, because the walk asks it what is
     * code.  After the labels, since the LD/LR entries it roots from are made
     * section-relative there. */
    if (reach_apply) reach_compute();
