...
#include "system.h"
#include "coretypes.h"
#include "tm.h"		/* for MAP_INCHAR (EBCDIC targets) */
#include "cpplib.h"
#include "cpphash.h"
#include "cppucnid.h"
		     ...
		     struct _cpp_strbuf *tbuf, bool wide)
{
  if (wide)
      ...
      if (tbuf->len + 1 > tbuf->asize)
	{
      ...
      /* A \x / \NNN escape is a literal byte value, not a source character,
	 so it must survive the host->EBCDIC translation that ASM_OUTPUT_ASCII
	 (MAP_OUTCHAR) applies to string bytes.  Pre-image it through MAP_INCHAR
	 so that round-trip leaves the literal value unchanged.  Identity on
	 non-EBCDIC targets.  */
      tbuf->text[tbuf->len++] = MAP_INCHAR (n);
    }
}
