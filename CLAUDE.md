...
| **xmit370** | host directory → TSO TRANSMIT of a **source** PDS, and back (`xmit370/src/xmit370.c`) | `create` packs samplib/JCL/macros as PO/RECFM=FB (CP037, ISPF statistics, reproducible with `--stats-date`); `list`/`extract` read any XMIT, including real TSO TRANSMIT and CBT files. Validated on host against a real TRANSMIT oracle (`ctca_demo.xmi`: DCB, directory split, statistics, every member = lines×80) and a 217-member byte-identical round trip. **End-to-end on real MVS (2026-08-13):** ufsd/ftpd/httpd samplibs built on the host, uploaded FB80, `RECEIVE`d into DCB-less targets -> allocated `PO/FB/80/3120` from the transmission itself, `IEB154I` per member, RC=0, content + ISPF statistics verified. Format delta: `docs/xmit-source-pds.md` |
...
- **mbt host-assembly/host-link backend — DONE (verified 2026-08-13).** mbt v2 already compiles, assembles *and* links entirely on the host: `mk/mbt.mk` sets `AS := as370` / `LD := ld370` with `%.o: %.asm|%.s` rules and `LINK_*` helpers, and `make deploy` (`scripts/mbtdeploy.py`) packs the per-module `.iebcopy` files with `ld370 --pack … -xmit`, uploads that one XMIT and RECEIVEs it. No ASM or LINK JCL is submitted any more — the only generated JCL left on the v2 path is the RECEIVE job and the `test-mvs` execution steps. The IEWL/IFOX00 helpers still in `scripts/mbt/jcl.py` + `datasets.py` serve `scripts/legacy/mvslink.py` only.
...
**`--pack` PDS2 directory (modlen + entry) — DONE & validated on MVS (2026-06-22).** `--pack` used to write `modlen=-1`/`entry=-1` per member → `build_userdata` echoed the template (PDS2STOR=8, PDS2EPA=0), so every packed member showed **SIZE 8** on MVS and lost its entry point. `modlen` (PDS2STOR) is now recomputed from the member's CESD (`member_modlen()` = the single-link's `roundup8(running)`). The **entry point is NOT in a bare `.lm`** (the single-link path only ever writes it to the directory), so `--pack` also accepts **single-member `-iebcopy`** inputs (the self-describing form), recovering the real entry + modlen from their directory (`read_iebcopy_member()`). Recommended workflow: build each module `ld370 -o NAME --name NAME obj... -iebcopy`, then `ld370 --pack A=A.iebcopy B=B.iebcopy -o lib -xmit`. Bare `.lm` inputs still work (modlen recomputed; entry assumed 0) and **since cc370#37 they say so** — a bare pack warns, naming the entry and the attributes actually applied, while packing the `-iebcopy` stays silent. The two halves are not equally repairable: `--ac`/`--norent`/`--noreus` given on the *pack* command do apply to a bare member, the entry point cannot be supplied at all (`--entry` is accepted by the parser and never reaches the pack path). Measured: two links differing only in entry point produce **byte-identical** bare members, with the whole difference in two directory bytes (PDS2EP0 and PDS2EPA). Validated: AMBLIST shows BIGENT `MAIN ENTRY POINT 0003E8` and it runs RC=7 (a wrong entry 0 would abend on the filler); packed single-member directory is byte-identical to the single-link `-iebcopy` directory.
...
**Correction (2026-06-25): the "byte-identical to IEWL" above is wrong.** I'd compared ld370 against a 799-byte copy of itself (`IBMUSER.FOO.LINKLIB`), not the real 858-byte `REF.LINKLIB(IRXTSPRM)`. The IEWL/IFOX reference has a **leading blank NULL CESD entry** (so IRXTSPRM is ESD-ID 2, RLD R=P=2) + **3 IDRs** (SPZAP + LK 5752SC104 + TRAN 5741SC103); ld370/as370 emits 1 CESD entry (ESD-ID 1, RLD R=P=1) + 1 SPZAP IDR. **Functionally equivalent (proven: the user's redeploy runs — `irxdbg anch`/IRXTMPW/IRXINIT all work), NOT byte-identical** — the +59 bytes are IFOX/IEWL cosmetic artifacts. The over-packing diagnosis & fix are correct; only the "byte-identical" wording was off. (True byte-identity would need as370 to emit the leading blank ESD + ld370 the LK/TRAN IDRs — a future verification nicety, not a functional need.)

**`--pack` >4 MB SIGBUS — fixed-buffer class closed in the write path (2026-06-25).** The `--pack` write path still used fixed 4 MB static buffers: `out[]` (link emit), `write_unload_mem`'s `unl[]`, `write_xmit`'s `unl[]`+`xm[]`. The 12-module production pack (~1.6 MB) fit; a 45-module `test-mvs` pack (~15.5 MB — each test embeds the full interpreter core) wrote ~11 MB past `unl[4 MB]` → **SIGBUS**. Fix: `unload_size()` computes the exact unload bytes (env + dir blocks + EOD + per-block `12+data` + per-member EOF); `write_unload_mem`/`write_xmit` `malloc` to that (xm bounds the NETDATA/FB80 framing as `usize + usize/32 + 64 KB`), and `out[]` (link path) now grows via `grow_arr` (a single huge module is safe too). Member bytes unchanged → redeploy-only. Verified: pre-fix SIGBUS (rc=138) on a 12×400 KB / 4.86 MB pack, post-fix rc=0; suite green incl. byte-identity. `run.sh` `>4MB pack` regression.

**Target BLKSIZE is runtime — `--blocksize`, default 15040 (2026-06-24).** The unloaded form's BLKSIZE was a hardcoded `19069` (a 3350 full track). A member built at 19069 only fits a *fresh* `>=19069` library; **15040** is the de-facto LINKLIB blocksize, so a member built there installs into ANY LINKLIB with BLKSIZE `>=15040`. `--blocksize N` (default 15040, range 1024..32760) now drives, consistently: the text-record split limit `maxtext`, the unloaded-PS BLKSIZE `UNLOAD_BLKSIZE` (= N+20), the COPYR1 `UBLKSIZE` (off 6 = the library BLKSIZE) and `off 14` (= the PS BLKSIZE), and the INMR02 `INMBLKSZ` (#1 = N, #2 = N+20). `maxtext` is the largest IEWL `TXTSIZE` table entry `<= N` (HEWLFINT `TXT18K..TXT1K`; 15040→13312, 19069→18432), keyed off the *library* BLKSIZE not the device max, so a library smaller than the device track still yields blocks that fit it. **Field mapping pinned empirically** against two real oracles — `e2e` (3350/19069) and CBT571 `LOADLIB` (3380/6144): COPYR1 off 6 == IEBCOPY `INMR02#1` `INMBLKSZ`, off 14 == INMCOPY `INMR02#2` `INMBLKSZ` (off 14 was a latent skew — baked 19069 but its own INMR02#2 declared MINBLK). **Pack-time guard:** `--pack` does not re-split (`split_member` reproduces *build-time* block sizes), so packing a member built at a larger `--blocksize` while declaring a smaller one is **refused on the host** (`member '…' has a N-byte block > --blocksize M`) — and since mbt v2 rebuilds on make timestamps (the SHA256 stamps in `scripts/mbt/stamps.py` hash *sources* and serve the legacy path only), an unchanged `.c` is neither recompiled nor relinked, so a stale large-block member is the likely first-deploy state. Tests: `run.sh` `--blocksize` (COPYR1 off6/off14 + max block + INMR02#1, default 15040 + 19069 + 6144 overrides) and the guard negative control. **Host-validated only** — the BLKSIZE/SB37 class needs the user's `mbt deploy` (RECEIVE self-alloc); RECV370 round-trips use the JCL SYSUT2 DCB and bypass INMR02/COPYR1.

Stage 3 is closed: the mbt host-link backend ships the XMIT instead of submitting ASM/LINK JCL (see the Cross-cutting entry above). (INMSIZE — `8a4f42c` — and the source-DCB/BLKSIZE — this change — are now computed, no longer fixed E2E constants.)
