...
const unsigned char mvs_unload_env_hdr[MVS_ENV_HDR_LEN] = {
...
/* compile-time guard: MAXBLK is written as a relationship, not a literal, so it
 * cannot drift from LEN/OVH -- but the value itself is load-bearing (it is the
 * 3350 UMBLK), so pin it. */
typedef char mvs_trk_maxblk_is_19069[(MVS_TRK_MAXBLK_3350 == 19069) ? 1 : -1];

void mvs_udebx_extent(unsigned char *hdr, int ntracks)
{
    int ncyl = (ntracks + MVS_TRKPERCYL_3350 - 1) / MVS_TRKPERCYL_3350;
    if (ncyl < 1) ncyl = 1;
    mvs_put16(hdr + MVS_UDEBX_ENDCC, MVS_UNLOAD_DATA_CC + ncyl - 1);
    mvs_put16(hdr + MVS_UDEBX_ENDHH, MVS_TRKPERCYL_3350 - 1);
    mvs_put16(hdr + MVS_UDEBX_NMTRK, ncyl * MVS_TRKPERCYL_3350);
}
